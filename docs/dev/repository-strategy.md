...
This document decides how Continuity VPN is published as open source and how the
...
- `internal/gateway/dataplane.go` imports `continuity-vpn/pkg/clientcore`.
- `internal/gateway/server.go` imports `continuity-vpn/internal/protocol`,
  `continuity-vpn/internal/dedup` and `continuity-vpn/internal/metrics`.
- `cmd/gateway` imports `continuity-vpn/internal/protocol`.
...
1. Extract a new module, e.g. `github.com/<org>/continuity-core`, containing the
...
| ------------------------------------ | ------------- | -------- |
| `continuity-core` (new shared module) | Apache-2.0    | `pkg/clientcore`, `pkg/protocoltypes`, `pkg/testkit`, and the shared logic currently in `internal/protocol`, `internal/dedup`, `internal/metrics` (re-exported as `pkg/…`) |
| `continuity-client`                  | Apache-2.0    | `apps/macos`, `bridge/`, `cmd/continuityctl`, client-only `internal/*` (`paths`, `platform`, `diagnostics`, `transport`, `bootstrap`) |
| `continuity-gateway`                 | AGPL-3.0-only | `cmd/gateway`, `internal/gateway`, gateway-only `deploy/` assets |
...
`<repo>` with the repository name (today `continuity-vpn`). The existing remote is
...
git ls-files | grep -E '(^|/)(gateway|continuityctl)$' || echo "clean: no built binaries tracked"
```
...
git rev-list --all --objects | grep -E '(^|/)(gateway|continuityctl)$' || echo "no built binary ever committed"
```
...
- Add a CI step asserting the licence invariant directly from the import graph,
  ...
  go list -deps ./cmd/continuityctl ./pkg/... \
    | grep -E 'continuity-vpn/(cmd/gateway|internal/gateway)' \
...
- Set up the `Release` workflow's permissions and any `GHCR` token for the
  gateway image (`ghcr.io/<org>/continuity-gateway`), replacing the README's
...
- [ ] No tracked built binaries (`gateway`, `continuityctl`).
...
- The built `gateway` binary and any `continuityctl` binary — rebuild from source.
